...
namespace API.Services.Api
{
    public class ApiService : IApiService
    {
        ...
        public ApiService(HttpClient httpClient, IConfiguration configuration,UserManager<User> userManager)
        {
            ...
            _flaskApiUrl1 = configuration["FlaskApi:Url1"];  // Učitavanje URL-a iz konfiguracije
            _flaskApiUrl2 = configuration["FlaskApi:Url2"];
            _flaskApiUrl3= configuration["FlaskApi:Url3"];
    ...
    public async Task<User?> PrepoznajLiceAsync()
{
     try
            ...
            try
        {
            ...
            var response = await _httpClient.GetAsync(_flaskApiUrl1+userId);
        ...
        catch (Exception ex)
        {
         ...
         public async Task<string> GameQuizzPlayAsync(string userId)
        {
            try
        {
            var content = new StringContent("{}", Encoding.UTF8, "application/json");
        ...
        catch (Exception ex)
        {
         ...
         public async Task<string> GameLeftPlayAsync(string userId)
        {
            try
        {
            var content = new StringContent("{}", Encoding.UTF8, "application/json");
            ...
            var response = await _httpClient.GetAsync(_flaskApiUrl3+userId);
